...
ggplot(data = data_mod) +
    geom_histogram(mapping = aes(x = Departure.Delay.in.Minutes),
                   bins = 30)
...
data_mod$Departure.Delay.in.Minutes = data_mod$Departure.Delay.in.Minutes +
    1
...
outliers = subset(
    data_mod,
    data_mod$Departure.Delay.in.Minutes > (Q1 - 1.5 * IQR) &
...
data_mod_1$zscore = (
    abs(
        data_mod_1$Departure.Delay.in.Minutes - mean(data_mod_1$Departure.Delay.in.Minutes)
    ) / sd(data_mod_1$Departure.Delay.in.Minutes)
)
...
#stepwise regression
model_all = glm(satisfaction ~ .,
                data = data_mod,
                family = binomial(link = "logit"))
model_intercept = glm(satisfaction ~ 1,
                      data = data_mod,
                      family = binomial(link = "logit"))

forward = step(
    model_intercept,
    scope = formula(model_all),
    direction = "forward",
    trace = 0
)
forward$anova
...
model = glm(
    satisfaction ~ Gender_Female + Customer.Type + Age + Type.of.Travel + Class +
        Inflight.wifi.service_1 + Inflight.wifi.service_3 + Inflight.wifi.service_4 +
        Inflight.wifi.service_5 +
        Departure..Arrival.time.convenient + Ease.of.Online.booking_1 +
        Ease.of.Online.booking_2 +
        Ease.of.Online.booking_3 + Ease.of.Online.booking_4 + Gate.location_1 +
        Gate.location_3 + Gate.location_4 +
        Gate.location_5 + Food.and.drink_1 + Food.and.drink_2 + Food.and.drink_4 +
        Online.boarding + Seat.comfort +
        Inflight.entertainment + On.board.service_1 + On.board.service_3 +
        On.board.service_4 + On.board.service_5 +
...
model = glm(
    satisfaction ~ Customer.Type + Age + Type.of.Travel + Class + Inflight.wifi.service_3 +
        Inflight.wifi.service_4 +
        Inflight.wifi.service_5 + Departure..Arrival.time.convenient +
        Ease.of.Online.booking_2 +
        Ease.of.Online.booking_3 + Ease.of.Online.booking_4 + Gate.location_3 +
        Gate.location_4 + Gate.location_5 +
        Food.and.drink_2 + Food.and.drink_4 + Online.boarding + Seat.comfort +
        Inflight.entertainment + On.board.service_3 +
        On.board.service_4 + On.board.service_5 + Leg.room.service +
